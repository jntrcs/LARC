...
files<-paste0("MHResults/",list.files("MHResults/"))
type<-factor()
...
acceptanceRates<-list(BradleyTerryGamma=vector("list", 5),Beta=vector("list", 5), ThurstoneMostellerNormal=vector("list", 5), ExtremeBT=vector("list",5))
...
for (i in 1:length(files))
{
  ...
  acceptanceRates[dat$TrueStrengthType][[1]][[a]]<-dat$AcceptanceRate
