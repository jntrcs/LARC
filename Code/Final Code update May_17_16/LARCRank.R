...
#  LARC.Optim
#
practicedf<-data.frame(colnames(c("Team", "Strength", "TotalWins","PlayedMatrix","BeatMatrix")))
aGames<-matrix(data=c(0,0,0,1),nrow=2, ncol=2, byrow=TRUE)
aWins<-matrix(data=c(0,0,0,1),nrow=2, ncol=2, byrow=TRUE)
bGames<-matrix(data=c(0,0,0,1),nrow=2, ncol=2, byrow=TRUE)
bWins<-matrix(data=c(0,0,0,0),nrow=2, ncol=2, byrow=TRUE)
 data<-c("Team A", 1,1,aGames, aWins)
