...
simulate1<-function(useBT, useBeta = FALSE, extremeBT=FALSE)
{
  ...
  for (i in 1:13)
  {
    ...
    acceptanceRate[[i]]<-list()
    datBT<-newMetHast(logBTDensity, nSamples = nsamp, winsMatrix = configured$WinsVersus, sig=2.1)
    acceptanceRate[[i]]$BT<-datBT[[2]]
    datBT<-handleBurnIn(datBT[[1]],200)
    meanStrengths[[i]]$BT<-analyzeMHMatrix(datBT)
    rm(datBT)
    datTM<-newMetHast(logTMDensity, nSamples = nsamp, winsMatrix = configured$WinsVersus, sig=2.1)
    acceptanceRate[[i]]$TM<-datTM[[2]]
    datTM<-handleBurnIn(datTM[[1]],200)
  ...
  summaryOfResults$AcceptanceRate<-list()
  summaryOfResults$AcceptanceRate$BT<-sapply(1:13, function(i)acceptanceRate[[i]]$BT)
  summaryOfResults$AcceptanceRate$TM<-sapply(1:13, function(i)acceptanceRate[[i]]$TM)
