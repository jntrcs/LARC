...
datascrape <- function(datatype="NBA",playoffs=FALSE,year=substr(Sys.Date(),1,4)){
  ...
  if(datatype=="NCAAB"){
    tables <- read.fwf(file=url("http://masseyratings.com/scores.php?s=284067&sub=12801&all=1&mode=3&sch=on&format=0"),
      skip=9, n=15369, widths=c(10, 2, 24, 3, 2, 24, 3, 10, 25),
      col.names=c("Date","Where1","Team1","PTS1","Where2","Team2","PTS2","OT?","Notes"))
    table1 <- read.fwf(file=url("http://masseyratings.com/scores.php?s=284067&sub=12801&all=1&mode=3&sch=on&format=0"),
                       skip=9, n=2, widths=c(19, 2, 24, 3, 2, 24, 3, 10, 16),
                       col.names=c("Date","Where1","Team1","PTS1","Where2","Team2","PTS2","OT?","Notes"))
...
#NFLScores <- datascrape("NFL",TRUE)
NCAAScores <- datascrape("NCAAB")

#this line if used provides the data types for each column of the data frame
#sapply(NBAScores,typeof)
