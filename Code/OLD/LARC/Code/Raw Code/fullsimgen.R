...
full.sim.gen <- function(teams, weeks, strengths=NULL, divisions, gid, god=NULL, enddate=Sys.Date(), type="BT") {
  ...
  for (j in 1:divisions) {
    for (i in 1:(tpd-1)) {
      Home <- c(Home,rep(divs[j,i],tpd*gid/2-i*gid/2))
      Visitor <- c(Visitor,rep(divs[j,(i+1):tpd],gid/2))
      Visitor <- c(Visitor,rep(divs[j,i],tpd*gid/2-i*gid/2))
      Home <- c(Home,rep(divs[j,(i+1):tpd],gid/2))
    }
  }
...
(try1pp <- progresspredict(try1))

try1pt <- try1pp$ProgressionTable

plot(try1pt$Week,try1pt$Mosteller)
points(try1pt$Week,try1pt$BradleyTerry,pch=3)

table(try1pt$Model)

try2 <- full.sim.gen(teams=30, weeks=10, strengths=NULL, divisions=3, gid=6, god=2, enddate=Sys.Date(), type="M")
#View(try1)

(try2pp <- progresspredict(try2))

try2pt <- try2pp$ProgressionTable

plot(try2pt$Week,try2pt$Mosteller)
points(try2pt$Week,try2pt$BradleyTerry,pch=3)
